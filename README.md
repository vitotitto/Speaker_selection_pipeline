...
Fields extracted but **deliberately redacted** from the LLM prompt:
- Name — used only by post-LLM deterministic guards (named-turn guard, label normalisation), never sent to the LLM

Fields provided to the LLM (non-identifying demographics only):
- Gender (if available)
- Birth year and death year (presented as "lived" range)

Fields extracted but **withheld** from the LLM to prevent diagnostic bias:
- Dementia type (dementia group only)
- First symptoms year (dementia group only)
- Language (if available)

The subject's name is deliberately withheld so the LLM classifies speakers based purely on conversational structure (who asks vs. answers, turn-taking, pronoun usage), not identity. The name is still used internally by the post-LLM named-turn guard and label normalisation steps, which operate deterministically on the raw transcript text.
...
- **Patient context block**: Non-identifying demographics (gender, birth/death years) — name is redacted
...
**Label normalisation**: Since the LLM does not receive the subject's name, it should always use the canonical label "subject". However, as a safety net, the pipeline still normalises "patient" to "subject" and checks for name matches (in case transcripts contain the name and the LLM echoes it).
